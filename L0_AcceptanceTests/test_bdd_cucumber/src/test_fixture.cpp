...
void TestFixture::SendSignal(int signal) {
  ...
  switch (signal) {
    ...
    case SIGQUIT:
      // Trigger termination
      comm::Terminate::Instance().TerminateApp();
      break;
    default:
...
void TestFixture::RunApplication() {
  try {
    ...
    // Initialize config
    comm::Config::Instance().Initialize("modu-core-test");
    if (!m_config_file.empty()) {
      comm::Config::Instance().Load(m_config_file);
    }
    TestHooks::OnModuleInitialized("comm::Config");

    // Initialize terminate handler
    auto error = comm::Terminate::Instance().Start();
    if (error) {
      LOG(ERROR) << "[TEST_FIXTURE] Failed to start terminate handler: " << error.message();
      m_exit_code = 1;
      m_running = false;
      return;
    }
    ...
    LOG(INFO) << "[TEST_FIXTURE] Application initialized, waiting for termination";

    // Wait for termination signal
    std::string reason = comm::Terminate::Instance().WaitForTermination();
    LOG(INFO) << "[TEST_FIXTURE] Termination received: " << reason;
